...
place — no saving — and `lake build LaxLogic.LJFOAudit` then completes in
...
**What this costs you.**  `lake build LaxLogic.LJFO` no longer re-checks the
...
/-! ## Part 4 results (from `LaxLogic.LJFOCore`)
...
/-! ## Part 5–8 results (from `LaxLogic.LJFO`)
