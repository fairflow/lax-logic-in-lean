...
/-! ### Guarded-list membership

The interpolant tables below are nested `if`s whose dead branches are
literally `[]`, and the proofs about them spend most of their length on
`split`. `mem_ite_list` is the characterisation that lets one `simp only`
collapse a cascade of any depth instead: the `[]` branches die under
`and_false`/`false_or`. Mathlib's `mem_ite` (`Mathlib/Logic/Basic.lean`) has
the shape `(p → a ∈ s) ∧ (¬p → a ∈ t)`, which does not collapse. -/

/-- Membership in a guarded list. `cases inst` rather than `by_cases`, so that
`Classical.choice` stays out of the axiom pins. -/
...
/-- The `Option`-valued guard, for the `Γ.filterMap` rows. -/
theorem ite_none_eq_some {α : Type} {c : Prop} [inst : Decidable c] {e x : α} :
    ((if c then none else some e) = some x) ↔ (¬c ∧ e = x) := by
  cases inst with
  | isTrue h => simp [h]
  | isFalse h => simp [h]

/-- Collapse a guarded interpolant-table row to its normal form.

The lemma list is chosen so that the `[]` branches die while the guard
propositions survive **unnormalised**: `not_or`, `not_not`, `not_and`, `ne_eq`
and `Decidable` normalisation are deliberately absent, because the producing
side feeds those same guards back to `rw [if_neg …, if_pos …]`. -/
macro "mem_tbl" loc:(Lean.Parser.Tactic.location)? : tactic =>
      ...
      · -- E1: Γ ⊢ interE p (fuel+1) Γ
        ...
        · -- the rule clauses
          ...
          | ifThen A' B =>
              ...
              | prop q =>
                  mem_tbl at hin
                  rcases hin with ⟨hq, rfl⟩ | ⟨_, _, rfl⟩
                  · have ih' := ihE (B :: Γ)
                    rw [List.toFinset_cons] at ih'
      ...
      · -- A1: interA p (fuel+1) Γ C, Γ ⊢ C
        ...
        · -- the context clauses
          ...
          | ifThen A' B =>
              ...
              | prop q =>
                  mem_tbl at hin
                  rcases hin with ⟨hg, rfl⟩ | ⟨_, _, rfl⟩
                  · refine G4s.impLProp
                      (Finset.mem_insert_of_mem (memF hFΓ))
...
`interE p 0 Γ = ⊤` is the weakest approximant and each unfolding
sharpens it; dually `interA p 0 Γ C = ⊥`.  The implication-shaped
clauses flip variance, which is exactly why the two directions are
proved together.  Everything at the list calculus, where `cut` and
`impL_adm` compose the per-clause implications.
-/

/-- Identity is admissible in `G4c` (via completeness from `SC`). -/
...
theorem imp_mono {X X' Y Y' : PLLFormula}
  ...
  refine G4c.impR ?_
  refine G4c.impL_adm (l₀ := [X]) (List.Perm.swap _ _ _) ?_ ?_
...
theorem inter_fuel_mono (p : String) : ∀ (fuel : Nat),
  ...
  | succ fuel ih =>
      obtain ⟨ihE, ihA⟩ := ih
      constructor
      · -- [interE p (fuel+2) Γ] ⊢ interE p (fuel+1) Γ
        intro Γ
        simp only [interE]
        ...
        · -- the rule clauses
          ...
          | or A B =>
              rcases List.mem_singleton.mp hin with rfl
              have h₁ := ihE (A :: Γ)
              simp only [interE] at h₁
              have h₂ := ihE (B :: Γ)
              simp only [interE] at h₂
              refine G4c.andAll_elim ?_ (or_mono h₁ h₂)
              exact List.mem_append.mpr (Or.inr (List.mem_flatMap.mpr
                ⟨A.or B, hFΓ, .head _⟩))
          | somehow χ =>
              rcases List.mem_singleton.mp hin with rfl
              have hE := ihE (χ :: Γ)
              simp only [interE] at hE
              refine G4c.andAll_elim ?_ (box_mono hE)
              exact List.mem_append.mpr (Or.inr (List.mem_flatMap.mpr
                ⟨χ.somehow, hFΓ, .head _⟩))
          | ifThen A' B =>
              cases A' with
              | prop q =>
                  mem_tbl at hin
                  rcases hin with ⟨hg, rfl⟩ | ⟨hg, hq, rfl⟩
                  · have hE := ihE (B :: Γ)
                    simp only [interE] at hE
                    refine G4c.andAll_elim ?_ hE
              ...
              | and A₁ B₁ =>
                  rcases List.mem_singleton.mp hin with rfl
                  have hE := ihE (A₁.ifThen (B₁.ifThen B) :: Γ)
                  simp only [interE] at hE
                  refine G4c.andAll_elim ?_ hE
                  exact List.mem_append.mpr (Or.inr (List.mem_flatMap.mpr
                    ⟨(A₁.and B₁).ifThen B, hFΓ, .head _⟩))
              | or A₁ B₁ =>
                  rcases List.mem_singleton.mp hin with rfl
                  have hE := ihE (A₁.ifThen B :: B₁.ifThen B :: Γ)
                  simp only [interE] at hE
                  refine G4c.andAll_elim ?_ hE
                  exact List.mem_append.mpr (Or.inr (List.mem_flatMap.mpr
...
theorem interE_fuel_mono_le (p : String) {f f' : Nat} (h : f ≤ f')
    (Γ : List PLLFormula) : G4c [interE p f' Γ] (interE p f Γ) := by
  induction h with
  | refl => exact G4c.iden (.head _)
  | @step m _ ih =>
      exact G4c.cut ((inter_fuel_mono p m).1 Γ)
        ((ih.weaken _).perm (List.Perm.swap _ _ _))

/-- Multi-step fuel monotonicity, ∀-side. -/
theorem interA_fuel_mono_le (p : String) {f f' : Nat} (h : f ≤ f')
    (Γ : List PLLFormula) (C : PLLFormula) :
    G4c [interA p f Γ C] (interA p f' Γ C) := by
  induction h with
  | refl => exact G4c.iden (.head _)
  | @step m _ ih =>
      exact G4c.cut ih
        ((((inter_fuel_mono p m).2 Γ C).weaken _).perm (List.Perm.swap _ _ _))
