...
`LaxLogic/Belief/Realisability.lean`, which imports this file).
...
* `realP` (`x ⊩ᵖ[Ev, κ, w] φ`) — **presented-strategy realisability**: the
  ...
  (`LaxLogic/Belief/Realisability.lean`) shows this presentation is *necessary*:
...
The obstruction (`realS_fullness_obstruction`, `LaxLogic/Belief/Realisability.lean`)
