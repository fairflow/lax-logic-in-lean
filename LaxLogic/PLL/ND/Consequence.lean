...
-- `Γ ⊢ φ` for a set `Γ` inside `PLLND` (the context type selects it), `Γ ⊢[SetDeriv] φ` elsewhere.
attribute [turnstile] SetDeriv
attribute [scoped turnstile_default] SetDeriv
