...
# generated by this campaign: scanning them would read our own report back
GENERATED = {"claim-reconciliation.md", "status-ledger.md", "claims.tsv",
             "dangling-triage-2026-09-18.md"}

files.append(os.path.join(REPO, "docs", "calculus-map.md"))
for fn in sorted(os.listdir(os.path.join(REPO, "docs"))):
    if fn.endswith(".md") and fn not in GENERATED:
...
def decls_in_window(lines, idx, radius=2):
    """Last resort: declaration names within `radius` lines either way.

    A citation on the claim's own line is evidence; one two lines away is a
    HINT — the prose often names the theorem in the next sentence, as
    `docs/calculus-map.md` does for focalization.  Rows found this way are
    marked `near` in the `how` column and must not be read as citations."""
    out = []
    for j in range(max(0, idx - radius), min(len(lines), idx + radius + 1)):
        if j == idx:
            continue
        for d in decls_on_line(lines[j]):
            if d not in out:
                out.append(d)
    return out
    ...
    for i, line in enumerate(lines):
        ...
        for m in VERDICT_RE.finditer(line):
            ...
            if not decls:
                ...
                how = "block" if decls else how
            if not decls:
                decls = decls_in_window(lines, i)
                how = "near" if decls else "none"
...
with open(OUT, "w", newline="", encoding="utf-8") as fh:
    ...
    w.writerow(["file", "line", "verdict", "decls", "how", "quote"])
    for r in rows:
        w.writerow([r["file"], r["line"], r["verdict"], r["decls"],
                    r.get("how", "line"), r["quote"]])
