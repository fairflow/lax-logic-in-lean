...
def eMinPRD (sat : SatE2RD p) :
    ∀ (todo done Δ : List Neg) (ψ : Neg) (seen : SeenR) (b : HeightBook seen),
      ...
      Sum (UpFrom (fun e => Inv (interpR p e todo done none seen :: Δ) [] j ψ))
          (EscD Δ seen b)
  | .up (.atom a) :: todo, done, Δ, ψ, seen, b, hP, hΔ, hψ, _, d =>
      match eMinPRD sat todo (.up (.atom a) :: done) Δ ψ seen b
        (ParkedCtxP.cons (ParkedNP.atom a) hP) hΔ hψ (d.wk subParkOut) with
      | .inr esc => .inr esc
      | .inl w => .inl (UpFrom.mk1 w.1 (fun e' he' => w.2 e' he'))
  | .up .fls :: _, _, _, _, _, _, _, _, _, _, d =>
      .inl (UpFrom.mk1 0 (fun _ _ => nBotElimJ _ (List.mem_cons_self ..) d))
  | .up (.or P Q) :: todo, done, Δ, ψ, seen, b, hP, hΔ, hψ, j, d =>
      match seqSumG (K := Δ) (seen := seen) (b := b)
              ...
              (fun b' hb' => eMinPRD sat (b' ++ todo) done Δ ψ seen b hP hΔ hψ
                 ((invUp (d.wk subHeadOut) b' hb').wk subChainIn)) with
      | .inr esc => .inr esc
      | .inl g =>
          .inl (UpFrom.mk1
  ...
  | .up (.down M) :: todo, done, Δ, ψ, seen, b, hP, hΔ, hψ, _, d =>
      match eMinPRD sat (M :: todo) done Δ ψ seen b hP hΔ hψ
        (((invUp (d.wk subHeadOut) [M] (by simp [invertPos]))).wk subChainIn) with
      | .inr esc => .inr esc
      | .inl w => .inl (UpFrom.mk1 w.1 (fun e' he' => w.2 e' he'))
  | .and M N :: todo, done, Δ, ψ, seen, b, hP, hΔ, hψ, _, d =>
      match eMinPRD sat (M :: N :: todo) done Δ ψ seen b hP hΔ hψ
        ((invAndHyp (d.wk subHeadOut)).wk (subChainIn (b := [M, N]))) with
      | .inr esc => .inr esc
      | .inl w => .inl (UpFrom.mk1 w.1 (fun e' he' => w.2 e' he'))
  | .imp .fls _ :: todo, done, Δ, ψ, seen, b, hP, hΔ, hψ, _, d =>
      match eMinPRD sat todo done Δ ψ seen b hP hΔ hψ
        (invImpFls (d.wk subHeadOut)) with
      | .inr esc => .inr esc
      | .inl w => .inl (UpFrom.mk1 w.1 (fun e' he' => w.2 e' he'))
  | .imp (.atom a) N :: todo, done, Δ, ψ, seen, b, hP, hΔ, hψ, _, d =>
      match eMinPRD sat todo (.imp (.atom a) N :: done) Δ ψ seen b
        (ParkedCtxP.cons (ParkedNP.qimp a N) hP) hΔ hψ (d.wk subParkOut) with
      | .inr esc => .inr esc
      | .inl w => .inl (UpFrom.mk1 w.1 (fun e' he' => w.2 e' he'))
  | .imp (.or Q₁ Q₂) N :: todo, done, Δ, ψ, seen, b, hP, hΔ, hψ, _, d =>
      match eMinPRD sat todo (.imp (.or Q₁ Q₂) N :: done) Δ ψ seen b
        (ParkedCtxP.cons (ParkedNP.oimp Q₁ Q₂ N) hP) hΔ hψ (d.wk subParkOut) with
      | .inr esc => .inr esc
      | .inl w => .inl (UpFrom.mk1 w.1 (fun e' he' => w.2 e' he'))
  | .imp (.down (.up P')) N :: todo, done, Δ, ψ, seen, b, hP, hΔ, hψ, _, d =>
      match eMinPRD sat todo (.imp (.down (.up P')) N :: done) Δ ψ seen b
        (ParkedCtxP.cons (ParkedNP.simp P' N) hP) hΔ hψ (d.wk subParkOut) with
      | .inr esc => .inr esc
      | .inl w => .inl (UpFrom.mk1 w.1 (fun e' he' => w.2 e' he'))
  | .imp (.down (.and M₁ M₂)) N :: todo, done, Δ, ψ, seen, b, hP, hΔ, hψ, _, d =>
      match eMinPRD sat todo (.imp (.down (.and M₁ M₂)) N :: done) Δ ψ seen b
        (ParkedCtxP.cons (ParkedNP.aimp M₁ M₂ N) hP) hΔ hψ (d.wk subParkOut) with
      | .inr esc => .inr esc
      | .inl w => .inl (UpFrom.mk1 w.1 (fun e' he' => w.2 e' he'))
  | .imp (.down (.imp Q' N')) N :: todo, done, Δ, ψ, seen, b, hP, hΔ, hψ, _, d =>
      match eMinPRD sat todo (.imp (.down (.imp Q' N')) N :: done) Δ ψ seen b
        (ParkedCtxP.cons (ParkedNP.dyk Q' N' N) hP) hΔ hψ (d.wk subParkOut) with
      | .inr esc => .inr esc
      | .inl w => .inl (UpFrom.mk1 w.1 (fun e' he' => w.2 e' he'))
  | .circ Q :: todo, done, Δ, ψ, seen, b, hP, hΔ, hψ, _, d =>
      match eMinPRD sat todo (.circ Q :: done) Δ ψ seen b
        (ParkedCtxP.cons (ParkedNP.box Q) hP) hΔ hψ (d.wk subParkOut) with
      | .inr esc => .inr esc
      | .inl w => .inl (UpFrom.mk1 w.1 (fun e' he' => w.2 e' he'))
  | .imp (.down (.circ Q')) N :: todo, done, Δ, ψ, seen, b, hP, hΔ, hψ, _, d =>
      match eMinPRD sat todo (.imp (.down (.circ Q')) N :: done) Δ ψ seen b
        (ParkedCtxP.cons (ParkedNP.cimp Q' N) hP) hΔ hψ (d.wk subParkOut) with
      | .inr esc => .inr esc
      | .inl w => .inl (UpFrom.mk1 w.1 (fun e' he' => w.2 e' he'))
  | [], done, Δ, ψ, seen, b, hP, hΔ, hψ, _, d =>
      ...
      | some (_, N, rest) =>
          match eMinPRD sat [N] rest Δ ψ seen b
            ...
            (invFireHyp (findFire_mem hf) d) with
          | .inr esc => .inr esc
          | .inl w =>
              .inl (UpFrom.mk1 w.1 (fun e' he' => by
                rw [interpRFire_eq hf none]; exact w.2 e' he'))
      | none => sat done Δ ψ seen b hf hP hΔ hψ d
...
def aMinPRD (sat : SatA2RD p) :
    ∀ (todo done Δ : List Neg) (G : Neg) (seen : SeenR) (b : HeightBook seen),
      ...
      Sum (UpFrom2 (fun e f => Inv (interpR p e todo done none seen :: Δ) [] .tru
          ...
          (EscD Δ seen b)
  | .up (.atom a) :: todo, done, Δ, G, seen, b, hP, hΔ, _, d =>
      match aMinPRD sat todo (.up (.atom a) :: done) Δ G seen b
        (ParkedCtxP.cons (ParkedNP.atom a) hP) hΔ (d.wk subParkOut) with
      | .inr esc => .inr esc
      | .inl w => .inl (UpFrom2.mk1 w.1 (fun e' f' he' hf' => w.2 e' f' he' hf'))
  | .up .fls :: _, _, _, _, _, _, _, _, _, _ =>
      .inl (UpFrom2.mk1 0 (fun _ _ _ _ => nBotElim _ (List.mem_cons_self ..)))
  | .up (.or P Q) :: todo, done, Δ, G, seen, b, hP, hΔ, j, d =>
      match seqSumG (K := Δ) (seen := seen) (b := b)
  ...
  | .up (.down M) :: todo, done, Δ, G, seen, b, hP, hΔ, _, d =>
      match aMinPRD sat (M :: todo) done Δ G seen b hP hΔ
        (((invUp (d.wk subHeadOut) [M] (by simp [invertPos]))).wk subChainIn) with
      | .inr esc => .inr esc
      | .inl w => .inl (UpFrom2.mk1 w.1 (fun e' f' he' hf' => w.2 e' f' he' hf'))
  | .and M N :: todo, done, Δ, G, seen, b, hP, hΔ, _, d =>
      match aMinPRD sat (M :: N :: todo) done Δ G seen b hP hΔ
        ((invAndHyp (d.wk subHeadOut)).wk (subChainIn (b := [M, N]))) with
      | .inr esc => .inr esc
      | .inl w => .inl (UpFrom2.mk1 w.1 (fun e' f' he' hf' => w.2 e' f' he' hf'))
  | .imp .fls _ :: todo, done, Δ, G, seen, b, hP, hΔ, _, d =>
      match aMinPRD sat todo done Δ G seen b hP hΔ (invImpFls (d.wk subHeadOut)) with
      | .inr esc => .inr esc
      | .inl w => .inl (UpFrom2.mk1 w.1 (fun e' f' he' hf' => w.2 e' f' he' hf'))
  | .imp (.atom a) N :: todo, done, Δ, G, seen, b, hP, hΔ, _, d =>
      match aMinPRD sat todo (.imp (.atom a) N :: done) Δ G seen b
        (ParkedCtxP.cons (ParkedNP.qimp a N) hP) hΔ (d.wk subParkOut) with
      | .inr esc => .inr esc
      | .inl w => .inl (UpFrom2.mk1 w.1 (fun e' f' he' hf' => w.2 e' f' he' hf'))
  | .imp (.or Q₁ Q₂) N :: todo, done, Δ, G, seen, b, hP, hΔ, _, d =>
      match aMinPRD sat todo (.imp (.or Q₁ Q₂) N :: done) Δ G seen b
        (ParkedCtxP.cons (ParkedNP.oimp Q₁ Q₂ N) hP) hΔ (d.wk subParkOut) with
      | .inr esc => .inr esc
      | .inl w => .inl (UpFrom2.mk1 w.1 (fun e' f' he' hf' => w.2 e' f' he' hf'))
  | .imp (.down (.up P')) N :: todo, done, Δ, G, seen, b, hP, hΔ, _, d =>
      match aMinPRD sat todo (.imp (.down (.up P')) N :: done) Δ G seen b
        (ParkedCtxP.cons (ParkedNP.simp P' N) hP) hΔ (d.wk subParkOut) with
      | .inr esc => .inr esc
      | .inl w => .inl (UpFrom2.mk1 w.1 (fun e' f' he' hf' => w.2 e' f' he' hf'))
  | .imp (.down (.and M₁ M₂)) N :: todo, done, Δ, G, seen, b, hP, hΔ, _, d =>
      match aMinPRD sat todo (.imp (.down (.and M₁ M₂)) N :: done) Δ G seen b
        (ParkedCtxP.cons (ParkedNP.aimp M₁ M₂ N) hP) hΔ (d.wk subParkOut) with
      | .inr esc => .inr esc
      | .inl w => .inl (UpFrom2.mk1 w.1 (fun e' f' he' hf' => w.2 e' f' he' hf'))
  | .imp (.down (.imp Q' N')) N :: todo, done, Δ, G, seen, b, hP, hΔ, _, d =>
      match aMinPRD sat todo (.imp (.down (.imp Q' N')) N :: done) Δ G seen b
        (ParkedCtxP.cons (ParkedNP.dyk Q' N' N) hP) hΔ (d.wk subParkOut) with
      | .inr esc => .inr esc
      | .inl w => .inl (UpFrom2.mk1 w.1 (fun e' f' he' hf' => w.2 e' f' he' hf'))
  | .circ Q :: todo, done, Δ, G, seen, b, hP, hΔ, _, d =>
      match aMinPRD sat todo (.circ Q :: done) Δ G seen b
        (ParkedCtxP.cons (ParkedNP.box Q) hP) hΔ (d.wk subParkOut) with
      | .inr esc => .inr esc
      | .inl w => .inl (UpFrom2.mk1 w.1 (fun e' f' he' hf' => w.2 e' f' he' hf'))
  | .imp (.down (.circ Q')) N :: todo, done, Δ, G, seen, b, hP, hΔ, _, d =>
      match aMinPRD sat todo (.imp (.down (.circ Q')) N :: done) Δ G seen b
        (ParkedCtxP.cons (ParkedNP.cimp Q' N) hP) hΔ (d.wk subParkOut) with
      | .inr esc => .inr esc
      | .inl w => .inl (UpFrom2.mk1 w.1 (fun e' f' he' hf' => w.2 e' f' he' hf'))
  | [], done, Δ, G, seen, b, hP, hΔ, j, d =>
      match hf : findFire done (splits done) with
      | some (_, N', rest) =>
          ...
          | .inl w =>
              .inl (UpFrom2.mk1 w.1 (fun e' f' he' hf' => by
                ...
                exact w.2 e' f' he' hf'))
      | none => sat done Δ G seen b hf hP hΔ d
