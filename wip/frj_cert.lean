...
def certFile (T : Search.Tab) (nm : String) (Γ : List Form) (φ G : Form) : String :=
  let stmt := match Γ with
    | [] => s!"theorem {nm} : ([] : List PLLFormula) ⊬ {srcPLL φ} :=\n" ++
            s!"  not_derivable_of_countermodel K_{nm} {nm}_force"
    | [γ] => s!"theorem {nm} : [{srcPLL γ}] ⊬ {srcPLL φ} :=\n" ++
             s!"  not_entails_of_countermodel K_{nm} {nm}_force"
    | _ => s!"theorem {nm} : ([] : List PLLFormula) ⊬ {srcPLL G} :=\n" ++
           s!"  not_derivable_of_countermodel K_{nm} {nm}_force"
  ...
  s!"theorem {nm}_force :\n" ++
  s!"    ¬ (K_{nm}).force (K_{nm}).root (ofPLL {srcPLL G}) := by decide\n\n" ++
  stmt ++ "\n\n" ++
