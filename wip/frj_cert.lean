...
was tried and rejected — `.falsePLL.somehow` is not a legal chain, and the
generated file failed to compile (caught by this tool's own Lean run). -/
def srcPLL : Form → String
  | .atom p => s!"(PLLFormula.prop \"{p}\")"
  | .bot => "PLLFormula.falsePLL"
  | .and a b => s!"(PLLFormula.and {srcPLL a} {srcPLL b})"
  | .or a b => s!"(PLLFormula.or {srcPLL a} {srcPLL b})"
  | .imp a b => s!"(PLLFormula.ifThen {srcPLL a} {srcPLL b})"
  | .circ a => s!"(PLLFormula.somehow {srcPLL a})"
...
def srcNamed (A : Form) : String :=
  match (List.range RNBank.reps.length).find?
          (fun k => match RNBank.reps[k]? with
  ...
  | none =>
    ...
    | .atom p => s!"(PLLFormula.prop \"{p}\")"
    | .bot => "PLLFormula.falsePLL"
    | .and a b => s!"({srcNamed a}.and {srcNamed b})"
    | .or a b => s!"({srcNamed a}.or {srcNamed b})"
    | .imp a b => s!"({srcNamed a}.ifThen {srcNamed b})"
    | .circ a => s!"({srcNamed a}.somehow)"
