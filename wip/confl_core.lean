...
`confluentB` — the decidable form of `PLLND.MutuallyConfluent`
(PLLFrames.lean:224) on the probe representation (`FinCM`, reflexive
closures built into `riB`/`rmB`) — plus the model/formula generators
of `wip/samval_probe.lean` and `wip/mforth_probe.lean`, copied
verbatim (those files each declare a top-level `main`, so they cannot
be imported by another executable root).  The transcription is
verified behaviourally: each refilter executable first re-runs the
UNRESTRICTED pass and must reproduce the pinned numbers
(samval one-atom 2,377,307 agreeing pairs, failures 499/44/12;
mforth 746,108 agreeing pairs, 0 candidates, 22,506 fallible-pair
failures).
-/
...
/-- Decidable mutual confluence on the probe representation: for all
`x, w, v` with `rmB x w` and `riB x v` (reflexive closures included,
exactly as the probes read `Ri`/`Rm`), some `u` has `riB w u` and
`rmB v u`. -/
def confluentB (M : FinCM) : Bool :=
  (List.range M.n).all fun x =>
...
/-! ## Generators, copied verbatim from `wip/samval_probe.lean`
(`= wip/mforth_probe.lean` where both have them) -/
...
/-- One same-trace probe pass (verbatim `SamValProbe.runPass`). -/
...
/-- Truth fingerprint on the variable-free list (mforth probe). -/
def finger (M : FinCM) (w : Nat) : List Bool :=
  formulas.map fun φ => M.forceB w φ

/-- The forth-m/fallible-pair pass of `wip/mforth_probe.lean`, with
fingerprints precomputed per point (a speed-up only; the counted
values are identical to the original inline computation). -/
def runForthM (label : String) (cms : List FinCM) : IO Unit := do
  IO.println s!"=== {label}: {formulas.length} formulas, {cms.length} frames ==="
  let mut pairs := 0
  let mut cands := 0
  let mut fcands := 0
  let tabs := cms.map fun M => (M, (List.range M.n).map (finger M))
  let pts := tabs.flatMap fun (M, fs) =>
    (List.range M.n).map fun w => (M, fs, w)
  for (M, fsM, w) in pts do
    let fw := fsM.getD w []
    for (N, fsN, w') in pts do
      if fw == fsN.getD w' [] then
        pairs := pairs + 1
  ...
  IO.println s!"=== {label}: agreeing pairs={pairs} forth-m candidates={cands} fallible-pair failures={fcands} ==="
