...
graph TD
  ...
  N4[N4 StabilisationAll · ◯-free PROVED, PLL OPEN · HALTED 2026-09-06, live obligations SatE2RW + SatA2RW §4.34] --> N5[N5 UI for LJF◯ · PROVED rel. N4]
  N3 --> N5
...
station contains a sub-derivation of the guard sequent).  **WP12 — Stage 0 DONE 2026-09-06 (§4.29, `docs/n4-pair-design.md`): the pair-recording `interpR` built (`wip/ui_routeB_r_def.lean`), termination kernel-certified at every designed cell (thresholds to 34), the escape property as designed, soundness and the validation cells not refuted; the run was killed by an external access error at R3 and recovered here. Stage 1 (the proof: `QBoundR`, soundness, cofinality with escapes ◯-free first, `hasUI_R`, N3 backward for `interpP`, `pll_ui_R`) relaunched 15:35 as WP12b, one agent: Stages 1–2 DONE and verified (§4.30) — `rBound` (literal stabilisation at EVERY station, unconditional) and `eSoundR`/`aSoundR` (by the easy halves for `interpR` and one cut each side); Stage 4 DONE and verified (§4.31): `pll_ui_R : (∀ p, SatE2P p) → (∀ p, SatA2P p) → (∀ p, SatE2R p) → (∀ p, SatA2R p) → PLL_UI` — uniform interpolation for PLL now rests on the COFINALITY OF THE PAIR RECURSION at saturated stations (`SatE2R`, `SatA2R`), Stage 3 delivered as DESIGN (§4.32): the escape-carrying statements `SatE2RE`/`SatA2RE` stated and PROVED to specialise to `SatE2R`/`SatA2R`; WP12c (§4.33) showed the formula-level escapes UNSUITABLE and replaced them by DERIVATION-level escapes (`EscD`, the height book, `BookBound`): record monotonicity, the confinement of the record extension, set-equal weakening, the recording-site loop, the processing phase at every record PROVED; `PLL_UI` now rests on `SatE2RD`/`SatA2RD` (`pll_ui_R_escD`). **WP12d (§4.34, 17:52–19:01): the family NOT built — `SatE2RD` REFUTED** (statement fault: record and p-free context quantified independently; refuted with a new one-world semantics for LJF◯, `no_inv_of_model`), repaired by booking the guard DERIVATION (`GuardBook`, `EscW`, `SatE2RW`/`SatA2RW`; `pll_ui_R_escW` PROVED, `refute_blocked`); the station-growth repair REFUTED (`growE_refuted`); the missing clause named: the escape must cross the p-free binders of the inversion phase (`EscBindOpenR`, OPEN; the kept-hypothesis crossings PROVED, `bindBackI`; goal-antecedent and disjunctive binders not crossed; one designed cell `BindCell`). **HALTED 2026-09-06 19:20 at Matthew's instruction**: results banked, no further builds; a record-context restatement (§4.34 assessment) is the first thing to test on any resumption.** the (antecedent, station-set) loop check;
