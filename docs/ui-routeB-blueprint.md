...
interpolation for PLL.  **WP11 (in flight, 08:25):** Stage 0c tests the
candidate induction hypotheses on the inner states by the decider; Stage 1
fuel monotonicity (no such lemma exists); Stage 2 the hard halves, the `∀p`
half by the direct induction, the `∃p` half with the dropped conjunct
carried in the hypothesis, ◯-free first; Stage 3 plumbs to `PLL_UI`.  If no
per-fuel hypothesis survives, the cofinal form is the honest restatement
and suffices for N4.
