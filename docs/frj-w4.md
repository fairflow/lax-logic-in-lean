...
### §14 second addendum: the `dn_circ_and` engine gap — located and closed (2026-08-17, chip session)

**Location.**  Not where the hand-trace above put it.  That trace is
not an FRJ derivation: the join over `{axI p, andI1(axI p), impInI(axI ⊥)}`
violates (J1) — the third premise's stable zone `{p,q}` is not covered by
`Σ_j ∪ Θ_j = {q, ¬¬(p∧q)}` of the first — and even a (J1)-legal family
containing the `impInI` cell forces `p, q` into the conclusion context
(`Σ^at` is kept), so it can only conclude `… ⇒ ⊥`.  The derivation the
PROVED construction (`Minimal.lean`, `minMod` on the 2-chain `r < b{p,q}`)
actually builds runs through `⊃∉` at the root world with zone
`Θ = nf(Λ*_r) = {¬¬(p∧q)}`:

```
axI ⊥                          ; p, q, ¬¬(p∧q) → ⊥
impInI  (Λ = {p,q,¬¬(p∧q)})    p, q, ¬¬(p∧q) ; → ¬(p∧q)         [Cl(Λ) ∋ p∧q]
⋈^At F=⊥                       p, q, ¬¬(p∧q) ⇒ ⊥                 [(J2): ¬(p∧q) ∈ Υ]
⊃∉  (Θ = {q, ¬¬(p∧q)})         ; q, ¬¬(p∧q) → ¬(p∧q)             [p∧q ∈ Cl(Γ) \ Cl(Θ)]
⋈^At F=p                       q, ¬¬(p∧q) ⇒ p                    [Θ^⊃/Υ keeps ¬¬(p∧q)]
andR1, impIn                   q, ¬¬(p∧q) ⇒ ¬¬(p∧q) ⊃ (p∧q)
```

The engine's `stepNotIn` offered only TWO `Θ`-candidates for `⊃∉`:
`Θmax = Cl(Γ) ∩ Ĝ` and `Θmax` purged of the SINGLE generators of `A`
(`X` with `A ∈ Cl({X})`).  With `A = p∧q` and `Θmax = {p, q, ¬¬(p∧q)}`
no single member generates `A`, both candidates are `Θmax`, `hAnot`
fails on both, and the row `; q, ¬¬(p∧q) → ¬(p∧q)` is never produced —
so the fixpoint (`r=4 RS=6 IS=11`, reproduced by hand row for row) is
reached without the goal.  The single-generator purge coincides with the
maximal admissible zones unless some conjunction inside `A` has all its
conjuncts generated by DISTINCT members of `Θmax` — no earlier corpus
antecedent had that shape (`¬¬p ⊃ p`: purge `p`), which is why the gap
stayed hidden until the compound-body cell; the byte-identical re-run
of every other cell confirms it.

**Fix** (`wip/frj_sat.lean`, `thetaCandidates`): enumerate the
⊆-maximal `Θ ⊆ Θmax` with `A ∉ Cl(Θ)`, read off the grammar of `Cl`
(`removalSets`: `∧` — drop one conjunct's generators; `∨` — both; `⊃`,
`◯` — the consequent/body's; plus `A` itself when `A ∈ Θmax`).  Maximal
zones suffice because every irregular consumer accepts a larger second
zone wherever it accepts a smaller one (the engine's `isLe` already
assumes this).  Side effect: `⊃∉` is now monotone in the regular
premise's context (every admissible zone over `Γ ⊆ Γ'` extends to a
maximal one over `Γ'`), so the RS-subsumption's one documented exception
(the `hAnot` gate) is gone.  `hAnot` is still decided at insertion:
the enumeration is untrusted and can only lose rows.

**Corpus re-run**: `dn_circ_and` flips to `transfer:pass` at BOTH bounds
(erasure `hit r=7 RS=8 IS=15`, decorated goal `hit r=8 RS=22 IS=25`);
every other line — verdicts AND `rounds/RS/IS` — is byte-identical to
the pre-fix run (33 corpus cells incl. 5 controls, 5 raised-budget
re-runs, the other 7 (E)-pairs).  (E) attack now 8/8 pass.  Runtime
55 s → 88 s, all of it the larger `dn_circ_and` saturations at
`jmax=4/pmax=3`.

**Method note.**  "engine-CERTAIN" (a rule-closure fixpoint below every
cap) is certain only relative to the COMPLETENESS of the enumeration
layer, not just its faithfulness (typing).  A `vacuous-CERTAIN` /
`FAIL-CANDIDATE` on a cell with a known countermodel is to be read as
"an enumerator is incomplete" first, and the diagnostic move is the one
that worked here: run the PROVED construction on the countermodel by
hand and find the first row the engine cannot produce.  The remaining
under-enumerations, both reported/cappable by design: `lamCandidates`
above `lamCap` (three-shape fallback, `Λcap` flag) and `seedsIC` above
`|Ĝ_at| = 4` (three valuations, currently NOT flagged in the stats).
