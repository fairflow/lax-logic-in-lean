/-
LJF◯ — the lax-flagged focused calculus, CORE (Parts 1–4).

Zero imports: no mathlib, no other calculus carries any of the proof.
This module holds the finished, pinned prefix — syntax, the four flagged
judgments, weights, the modal interpolant `interp` with its termination,
...
/-- The one-step self witness: `↑P ⊢ ↑P`, at either flag. -/
def upSelf (P : Pos) (j : JD) : Inv [Neg.up P] [] j (.up P) :=
  .stable (.lfoc (List.mem_cons_self ..) (.rel (idPos P _ _)))
      ...
      | some G =>
        ...
        | .up (.down M) =>
            nOrAll ([interp p [] done (some M)] ++
              ...
              | _, _ => nBot))
        | .circ (.atom q) =>
            .circ (.down (nOrAll ([interp p [] done (some (.up (.atom q)))] ++
              (splits done).attach.map (fun ⟨(X, rest), hXr⟩ =>
              match X, hXr with
              | .imp (.atom a) N, hXr =>
                  pGuard p a nBot
              ...
              | _, _ => nBot))))
        | .circ .fls =>
            .circ (.down (nOrAll ([interp p [] done (some (.up .fls))] ++
              (splits done).attach.map (fun ⟨(X, rest), hXr⟩ =>
              match X, hXr with
              | .imp (.atom a) N, hXr =>
                  pGuard p a nBot
              ...
              | _, _ => nBot))))
        | .circ (.or P₁ P₂) =>
            .circ (.down (nOrAll ([interp p [] done (some (.circ P₁)),
        ...
        | .circ (.down (.circ P')) =>
            .circ (.down (nOrAll ([interp p [] done (some (.circ P'))] ++
              ...
              | _, _ => nBot))))
        | .circ (.down (.and M₁ M₂)) =>
            .circ (.down (nOrAll ([interp p [] done (some (.up (.down (.and M₁ M₂))))] ++
              (splits done).attach.map (fun ⟨(X, rest), hXr⟩ =>
              match X, hXr with
              | .imp (.atom a) N, hXr =>
                  pGuard p a nBot
              ...
              | _, _ => nBot))))
        | .circ (.down (.imp Q₀ N₀)) =>
            .circ (.down (nOrAll ([interp p [] done (some (.up (.down (.imp Q₀ N₀))))] ++
              (splits done).attach.map (fun ⟨(X, rest), hXr⟩ =>
              match X, hXr with
              | .imp (.atom a) N, hXr =>
                  pGuard p a nBot
...
theorem nOr_inj {a b c d : Neg} (h : nOr a b = nOr c d) : a = c ∧ b = d := by
  simp only [nOr, Neg.up.injEq, Pos.or.injEq, Pos.down.injEq] at h
  exact h

theorem nOrAll_inj : ∀ {l₁ l₂ : List Neg}, nOrAll l₁ = nOrAll l₂ → l₁ = l₂
  | [], [], _ => rfl
  | [], _ :: _, h => by simp [nOrAll, nOr, nBot] at h
  | _ :: _, [], h => by simp [nOrAll, nOr, nBot] at h
  | _ :: _, _ :: _, h => by
      obtain ⟨h1, h2⟩ := nOr_inj h
      rw [h1, nOrAll_inj h2]
...
/-- The `hX` argument of an attack row: the split's head, read back through a
`done`-inclusion and past the row's own head hypothesis `A`. -/
...
/-- The `hrest` argument of an attack row: the split's residual, read back
through a `done`-inclusion and past the row's own head hypothesis `A`. -/
...
macro "ljf_dec_e" : tactic => `(tactic| (
    ...
    all_goals
      first
        ...
        | omega
        | (have h1 := p3_pos (wPos Q + 1); omega)
        | (have h1 := p3_pos (wPos Q' + 1 + 1 + wNeg N + 1); omega)
        | (have h1 := dec_boxF (by assumption); omega)
        | (have h1 := dec_cimpF (Q' := Q') (N := N) (by assumption); omega)
        | (have h1 := dec_cimpF (Q' := Q') (N := N_c) (by assumption); omega)
        | (have h1 := p3_succ (wPos P); have h2 := p3_pos (wPos P); omega)
        | (have h1 := p3_succ (wPos P'); have h2 := p3_pos (wPos P'); omega)
        | (have h1 := p3_succ (wPos P' + 1); have h2 := p3_pos (wPos P' + 1)
           omega)
      ...
      | none =>
          ...
          if hx1 : x ∈ [interp p [] done (some (.up (.atom q)))] then
            ...
            exact .circR (.stable (.laxOf (unStable
              ((aSound p [] done (.up (.atom q))).wk (Sub.cons _ hsubD)))))
          else
          have hx2 : x ∈ (splits done).attach.map _ :=
            (List.mem_append.mp hx).resolve_left hx1
          obtain ⟨⟨⟨X, rest⟩, hXr⟩, hmem, hEq⟩ := memMapWitness _ _ x hx2
  ...
  | [], done, .circ .fls => by
      ...
      | none =>
          ...
          if hx1 : x ∈ [interp p [] done (some (.up .fls))] then
            rcases List.mem_singleton.mp hx1 with rfl
            exact .circR (.stable (.laxOf (unStable
              ((aSound p [] done (.up .fls)).wk (Sub.cons _ hsubD)))))
          else
          have hx2 : x ∈ (splits done).attach.map _ :=
            (List.mem_append.mp hx).resolve_left hx1
          obtain ⟨⟨⟨X, rest⟩, hXr⟩, hmem, hEq⟩ := memMapWitness _ _ x hx2
      ...
      | none =>
          ...
          if he1 : x = interp p [] done (some (.circ P₁)) then
            ...
            exact .circR (.stable (stabOr1 (unStable (circROf
                ((aSound p [] done (.circ P₁)).wk (Sub.cons _ hsubD))))))
            ...
            subst he3
            exact .circR (.stable (.laxOf (unStable
              ((aSound p [] done (.up (.or P₁ P₂))).wk (Sub.cons _ hsubD)))))
          else
  ...
  | [], done, .circ (.down (.and M₁ M₂)) => by
      ...
      | none =>
          ...
          if hx1 : x ∈ [interp p [] done (some (.up (.down (.and M₁ M₂))))] then
            rcases List.mem_singleton.mp hx1 with rfl
            exact .circR (.stable (.laxOf (unStable
              ((aSound p [] done (.up (.down (.and M₁ M₂)))).wk (Sub.cons _ hsubD)))))
          else
          have hx2 : x ∈ (splits done).attach.map _ :=
            (List.mem_append.mp hx).resolve_left hx1
          obtain ⟨⟨⟨X, rest⟩, hXr⟩, hmem, hEq⟩ := memMapWitness _ _ x hx2
  ...
  | [], done, .circ (.down (.imp Q₀ N₀)) => by
      ...
      | none =>
          ...
          if hx1 : x ∈ [interp p [] done (some (.up (.down (.imp Q₀ N₀))))] then
            rcases List.mem_singleton.mp hx1 with rfl
            exact .circR (.stable (.laxOf (unStable
              ((aSound p [] done (.up (.down (.imp Q₀ N₀)))).wk (Sub.cons _ hsubD)))))
          else
          have hx2 : x ∈ (splits done).attach.map _ :=
            (List.mem_append.mp hx).resolve_left hx1
          obtain ⟨⟨⟨X, rest⟩, hXr⟩, hmem, hEq⟩ := memMapWitness _ _ x hx2
