...
theorem gbuSuccAtF {G : Form} {D : FSeq → Prop} (hsat : Saturated G D)
  ...
  obtain ⟨s', hs'mem, hsub⟩ :=
    hsat.2 (.reg (joinCtxAtF Ξ Θ f F) F)
...
theorem gbuSuccOrF {G : Form} {D : FSeq → Prop} (hsat : Saturated G D)
  ...
  have hJ2 : ∀ A B : Form,
    ...
    exact (E.spec A).mpr (List.mem_cons_of_mem _ (List.mem_cons_of_mem _
      (List.mem_map.mpr ⟨.imp A B, List.mem_filter.mpr ⟨hAB, rfl⟩, rfl⟩)))
  obtain ⟨s', hs'mem, hsub⟩ :=
    hsat.2 (.reg (joinCtxOrF Ξ Θ f) (.or C₁ C₂))
...
theorem refutedCleanly_circ {G : Form} {D : FSeq → Prop} (hsat : Saturated G D)
    ...
    (hz : EvalI D Ω Z) :
    RefutedCleanly G Ω (.circ Z) :=
  let ⟨Γ, d, hcov⟩ := refutedCleanly_circ_core circRuleV hΩ hgoal
...
theorem gbuSuccCircP {G : Form} {D : FSeq → Prop} (hsat : Saturated G D)
  ...
  have hJ2 : ∀ A B : Form,
    ...
    exact (E.spec A).mpr (List.mem_cons_of_mem _
      (List.mem_map.mpr ⟨.imp A B, List.mem_filter.mpr ⟨hAB, rfl⟩, rfl⟩))
  have hJ5 : ∀ Y : Form,
      Form.circ Y ∈ unionAll (fun j => circPart (Ξ j)) →
      ∃ _i : Fin 1, Clo Δ Y := by
    intro Y hY
    obtain ⟨j, hj⟩ := mem_unionAll.mp hY
    exact ⟨0, hreal Y (hStΩ j (List.mem_filter.mp hj).1)⟩
  have hJ7s : ∀ _i : Fin 1, ∀ j, ∀ X ∈ Ξ j, Clo Δ X :=
    fun _ j X hX => hcov X (hStΩ j hX)
  obtain ⟨s', hs'mem, hsub⟩ :=
    hsat.2 (.reg (joinCtxOrP Ξ Θ f (fun _ : Fin 1 => Δ)) (.circ Z))
...
/-! ### The two zone enlargements of clause 14

Clause 14 is an ELIMINATION of the irregular family, so it is not
hoisted: `cases d` needs the constructor set, and FRJV's and FRJW's
differ (`impNotIn`/`liftI` against `lift`).  What the arms DO is
calculus-free, and the two lemmas below carry it for both families —
`liftZoneGrow` for the `◯∉` and `Lift` arms (which re-admit at the
enlarged zone), `vacZoneGrow` for `Ax^I◯` (whose zone already contains
`Ω`, classically).  With `evalI_of_irr` supplying the (DB2) tail, each
arm is then three lines in each family. -/

/-- The zone enlargement `◯∉` and `Lift` re-supply: everything `Ω`
adds to the retained zone is still covered by the regular context, and
still in `Ĝ`. -/
theorem liftZoneGrow {G : Form} {Ω Ω' Γ Θ : List Form}
    (hΩ : ∀ X ∈ Ω, X ∈ gHat G) (hcl : ∀ X ∈ Ω, Clo Ω' X)
    (h2 : Ω' ⊆ [] ++ Θ) (hTh : ∀ X ∈ Θ, Clo Γ X ∧ X ∈ gHat G) :
    ∀ X ∈ Ω ++ Θ, Clo Γ X ∧ X ∈ gHat G := by
  intro X hX
...
/-- `Ax^I◯`'s zone already contains `Ω`: the zone is `Ĝ` filtered by a
classical valuation, and `Clo` preserves that valuation. -/
theorem vacZoneGrow {G : Form} {Ω Ω' Θ ats : List Form}
    (hΩ : ∀ X ∈ Ω, X ∈ gHat G) (hcl : ∀ X ∈ Ω, Clo Ω' X)
    (h2 : Ω' ⊆ [] ++ Θ) (hThv : Θ ≐ vacZoneA G ats) : Ω ⊆ [] ++ Θ := by
  intro x hx
...
theorem gbuInv14 {G : Form} {D : FSeq → Prop} (hsat : Saturated G D)
  ...
  obtain ⟨Ξ, Θ, hmem, h1, h2⟩ := h
  obtain ⟨d⟩ := hsat.1 (.irr Ξ Θ (.circ Z)) hmem
  cases d with
  ...
  | liftI dr hTh =>
      ...
      exact evalI_of_irr hsat
        ⟨.liftI dr (liftZoneGrow hΩ hcl h2 hTh)⟩
        (fun {_} hx => absurd hx List.not_mem_nil)
        (fun {_} hx => List.mem_append_right _ (List.mem_append_left _ hx))
...
private theorem evalI_circ_lift {G : Form} {D : FSeq → Prop} (hsat : Saturated G D)
  ...
  obtain ⟨Ξ, Θ, hmem, h1, h2⟩ := h
  obtain ⟨d⟩ := hsat.1 (.irr Ξ Θ (.circ C')) hmem
  cases d with
  ...
  | circNotIn dr htag hTh hg =>
      ...
      obtain ⟨s', hs'mem, hsub⟩ :=
        hsat.2 (.irr [] Θ (.circ C)) ⟨.circNotIn dr' htag' hTh hgoal⟩
      match s', hsub with
      | .irr Ξ' Θ' _, ⟨rfl, hSt, hTh'⟩ =>
          exact ⟨Ξ', Θ', hs'mem,
            fun {x} hx => absurd ((hSt x).mpr hx) List.not_mem_nil,
