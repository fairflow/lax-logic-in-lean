...
/-- `⋃_{1≤j≤n} f j`. -/
def unionAll {n : Nat} (f : Fin (n + 1) → Finset Form) : Finset Form :=
  Finset.univ.biUnion f

@[simp] theorem mem_unionAll {n : Nat} {f : Fin (n + 1) → Finset Form} {x : Form} :
...
def interAll {n : Nat} (f : Fin (n + 1) → Finset Form) : Finset Form :=
  (f 0).filter (fun x => ∀ j, x ∈ f j)

@[simp] theorem mem_interAll {n : Nat} {f : Fin (n + 1) → Finset Form} {x : Form} :
    x ∈ interAll f ↔ ∀ j, x ∈ f j := by
  simp only [interAll, Finset.mem_filter]
  exact ⟨fun h => h.2, fun h => ⟨h 0, h⟩⟩

/-- Membership test for the restriction operator. -/
def inRestrict (Υ : Finset Form) : Form → Bool
  | .imp A _ => decide (A ∈ Υ)
  | _ => false
...
`Γ⊃` to `Υ`." -/
def restrict (X Υ : Finset Form) : Finset Form :=
  X.filter (fun f => inRestrict Υ f = true)

theorem mem_restrict {X Υ : Finset Form} {A B : Form} :
    Form.imp A B ∈ restrict X Υ ↔ (Form.imp A B ∈ X ∧ A ∈ Υ) := by
  simp [restrict, inRestrict]
...
inductive FRJr (G : Form) : Finset Form → Form → Type
  /-- `Ax^R`:  `⊢ Ĝ_at \ {F} ⇒ F`,  `F ∈ Prime`. -/
  | axR (F : Form) (hF : F.isPrime) (hgoal : F ∈ sfR G) :
      FRJr G ((gAt G).erase F) F
...
def Provable (G : Form) : Prop := ∃ Γ : Finset Form, Nonempty (FRJr G Γ G)
