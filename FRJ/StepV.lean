/-
V-adaptation of `FRJ/Step.lean`; the three barren joins carry a kept
zone — see `FRJ/CalculusV.lean`.
...
properties of FRJ(G)-derivations", transported to the repaired family
`FRJVr`/`FRJVi`:

* `σ₁ ↦_R σ₂` iff `R` is a rule of `FRJV(G)` such that `σ₂` is the
...
The only change against the paper family: the three barren joins
`⋈^At`, `⋈^∨`, `⋈^◯` conclude with the context
`joinCtxAtVBase stab th F ++ kept` (resp. `joinCtxOrVBase stab th ++
kept`), where `kept` carries a `KeptChain` certificate.  Lemma 3.4 goes
through because every kept member lies in `Θ^⊃∩ ⊆ th j` for EVERY
premise index `j` (`keptChain_subset` + `interAll_subset`), so the kept
zone is contained in each premise's left formulas outright — an easier
inclusion than the old restricted second zone, not a harder one.

`Sequent`, `RuleName`, and the context-shape lemmas about the promise
and fallible join contexts are reused from `FRJ.Step` verbatim (they do
not mention derivations); only what recurses over derivations or touches
the three changed contexts is re-proved here.
-/
...
/-! ## `↦_R`, `↦₀`, `↦*` for the repaired family -/

/-- `σ₁ ↦_R σ₂`: `σ₂` is the conclusion of an instance of rule `R` of
`FRJV(G)` and `σ₁` is one of its premises.  Only the side conditions
...
inductive Step (G : Form) : RuleName → Sequent → Sequent → Prop
  ...
  | joinAt {n : Nat} {stab th : Fin (n + 1) → List Form}
      ...
      Step G .joinAt (.irr (stab j) (th j) (rhs j)) (.reg Γ' F)
  ...
  | joinOr {n : Nat} {stab th : Fin (n + 1) → List Form}
      ...
      Step G .joinOr (.irr (stab j) (th j) (rhs j)) (.reg Γ' (.or C₁ C₂))
  | joinAtP {n k : Nat} {stab th : Fin (n + 1) → List Form}
      ...
      Step G .joinAtP (.irr (stab j) (th j) (rhs j)) (.reg Γ' F)
  ...
  | joinAtF {n : Nat} {stab th : Fin (n + 1) → List Form}
      ...
      Step G .joinAtF (.irr (stab j) (th j) (rhs j)) (.reg Γ' F)
  | joinOrP {n k : Nat} {stab th : Fin (n + 1) → List Form}
      ...
      Step G .joinOrP (.irr (stab j) (th j) (rhs j)) (.reg Γ' (.or C₁ C₂))
  ...
  | joinOrF {n : Nat} {stab th : Fin (n + 1) → List Form}
      ...
      Step G .joinOrF (.irr (stab j) (th j) (rhs j)) (.reg Γ' (.or C₁ C₂))
  /-- the V-join `⋈^◯`: the conclusion context is base + kept. -/
  | joinCirc {n : Nat} {stab th : Fin (n + 1) → List Form}
      ...
      (hkc : KeptChain (upsilon rhs) (joinCtxOrVBase stab th)
        (thPool th) kept)
      {Γ' : List Form} (hΓ : Γ' ≐ joinCtxOrVBase stab th ++ kept) :
      Step G .joinCirc (.irr (stab j) (th j) (rhs j)) (.reg Γ' (.circ Z))
  | joinCircP {n k : Nat} {stab th : Fin (n + 1) → List Form}
      {rhs : Fin (n + 1) → Form} {Z : Form} {Δs : Fin (k + 1) → List Form}
      (j : Fin (n + 1))
      (hJ1 : ∀ i j, i ≠ j → stab i ⊆ stab j ++ th j)
      {Γ' : List Form} (hΓ : Γ' ≐ joinCtxOrP stab th rhs Δs) :
      Step G .joinCircP (.irr (stab j) (th j) (rhs j)) (.reg Γ' (.circ Z))
...
kept zone lies in `Θ^⊃∩ ⊆ th j` for EVERY `j`, which is a strictly
simpler inclusion than the old restricted zone's. -/

/-- Every member of the retention pool `Θ^⊃∩` is in every premise's
second zone. -/
theorem thPool_subset {n : Nat} {th : Fin (n + 1) → List Form}
    (j : Fin (n + 1)) : thPool th ⊆ th j :=
  fun _ hx => interAll_subset j (impPart_subset hx)

theorem joinCtxAtVBase_subset {n : Nat} {stab th : Fin (n + 1) → List Form}
    {F : Form}
    (hJ1 : ∀ i j, i ≠ j → stab i ⊆ stab j ++ th j) (j : Fin (n + 1)) :
    joinCtxAtVBase stab th F ⊆ stab j ++ th j := by
  intro x hx
...
theorem joinCtxOrVBase_subset {n : Nat} {stab th : Fin (n + 1) → List Form}
    (hJ1 : ∀ i j, i ≠ j → stab i ⊆ stab j ++ th j) (j : Fin (n + 1)) :
    joinCtxOrVBase stab th ⊆ stab j ++ th j := by
  intro x hx
...
the kept zone because `kept ⊆ Θ^⊃∩ ⊆ th j`. -/
theorem joinCtxAtV_subset {n : Nat} {stab th : Fin (n + 1) → List Form}
    {F : Form} {kept : List Form}
    (hJ1 : ∀ i j, i ≠ j → stab i ⊆ stab j ++ th j)
    (hkept : kept ⊆ thPool th) (j : Fin (n + 1)) :
    joinCtxAtVBase stab th F ++ kept ⊆ stab j ++ th j := by
  intro x hx
  rcases List.mem_append.mp hx with hx | hx
  · exact joinCtxAtVBase_subset hJ1 j hx
  · exact List.mem_append_right _ (thPool_subset j (hkept hx))

theorem joinCtxOrV_subset {n : Nat} {stab th : Fin (n + 1) → List Form}
    {kept : List Form}
    (hJ1 : ∀ i j, i ≠ j → stab i ⊆ stab j ++ th j)
    (hkept : kept ⊆ thPool th) (j : Fin (n + 1)) :
    joinCtxOrVBase stab th ++ kept ⊆ stab j ++ th j := by
  intro x hx
  rcases List.mem_append.mp hx with hx | hx
...
inductive OccR {G : Form} : {t : Tag} → {Γ : List Form} → {C : Form} →
    FRJVr G t Γ C → Sequent → Prop
  | root {t : Tag} {Γ : List Form} {C : Form} (d : FRJVr G t Γ C) : OccR d (.reg Γ C)
  | andR1 {t : Tag} {Γ : List Form} {A₁ A₂ : Form} {d : FRJVr G t Γ A₁}
      {hg : Form.and A₁ A₂ ∈ sfR G} {s : Sequent} :
      OccR d s → OccR (FRJVr.andR1 d hg) s
  | andR2 {t : Tag} {Γ : List Form} {A₁ A₂ : Form} {d : FRJVr G t Γ A₂}
      {hg : Form.and A₁ A₂ ∈ sfR G} {s : Sequent} :
      OccR d s → OccR (FRJVr.andR2 d hg) s
  | impIn {t : Tag} {Γ : List Form} {A B : Form} {d : FRJVr G t Γ B} {hA : Clo Γ A}
      {hg : Form.imp A B ∈ sfR G} {s : Sequent} :
      OccR d s → OccR (FRJVr.impIn d hA hg) s
  | circIn {t : Tag} {Γ : List Form} {Z : Form} {d : FRJVr G t Γ Z}
      {htag : t = .barren ∨ ∃ W, t = .chain W ∧ Covers Γ W Z}
      {hg : Form.circ Z ∈ sfR G} {s : Sequent} :
      OccR d s → OccR (FRJVr.circIn d htag hg) s
  | joinAt {n : Nat} {stab th : Fin (n + 1) → List Form}
      ...
      {hΓ : Γ' ≐ joinCtxAtVBase stab th F ++ kept} {s : Sequent} (j : Fin (n + 1)) :
      OccI (prem j) s → OccR (FRJVr.joinAt prem hJ1 hJ2 hcirc hkc hF hFnot hg hΓ) s
  | joinAtP {n k : Nat} {stab th : Fin (n + 1) → List Form}
      ...
      OccI (prem j) s →
      OccR (FRJVr.joinAtP prem dps hJ1 hJ2 hJ5 hJ7s htag hF hFnot hg hΓ) s
  ...
  | joinAtF {n : Nat} {stab th : Fin (n + 1) → List Form}
      ...
      OccI (prem j) s → OccR (FRJVr.joinAtF prem hJ1 hJ2 hF hFnot hg hΓ) s
  | joinOr {n : Nat} {stab th : Fin (n + 1) → List Form}
      ...
      {hΓ : Γ' ≐ joinCtxOrVBase stab th ++ kept} {s : Sequent} (j : Fin (n + 1)) :
      OccI (prem j) s → OccR (FRJVr.joinOr prem hJ1 hJ2 hcirc hkc hC hg hΓ) s
  | joinOrP {n k : Nat} {stab th : Fin (n + 1) → List Form}
      ...
      OccI (prem j) s →
      OccR (FRJVr.joinOrP prem dps hJ1 hJ2 hJ5 hJ7s htag hC hg hΓ) s
  ...
  | joinOrF {n : Nat} {stab th : Fin (n + 1) → List Form}
      ...
      OccI (prem j) s → OccR (FRJVr.joinOrF prem hJ1 hJ2 hC hg hΓ) s
  | joinCirc {n : Nat} {stab th : Fin (n + 1) → List Form}
      ...
      {hΓ : Γ' ≐ joinCtxOrVBase stab th ++ kept} {s : Sequent} (j : Fin (n + 1)) :
      OccI (prem j) s → OccR (FRJVr.joinCirc prem hJ1 hJ2 hcirc hkc hZ hg hΓ) s
  | joinCircP {n k : Nat} {stab th : Fin (n + 1) → List Form}
      ...
      OccI (prem j) s →
      OccR (FRJVr.joinCircP prem dps hJ1 hJ2 hJ5 hJ7s hDs hZ hg hΓ) s
...
inductive OccI {G : Form} :
    {St Th : List Form} → {C : Form} → FRJVi G St Th C → Sequent → Prop
  | root {St Th : List Form} {C : Form} (d : FRJVi G St Th C) : OccI d (.irr St Th C)
  | andI1 {St Th : List Form} {A₁ A₂ : Form} {d : FRJVi G St Th A₁}
      {hg : Form.and A₁ A₂ ∈ sfR G} {s : Sequent} :
      OccI d s → OccI (FRJVi.andI1 d hg) s
  | andI2 {St Th : List Form} {A₁ A₂ : Form} {d : FRJVi G St Th A₂}
      {hg : Form.and A₁ A₂ ∈ sfR G} {s : Sequent} :
      OccI d s → OccI (FRJVi.andI2 d hg) s
  | orI₁ {St₁ Th₁ St₂ Th₂ : List Form} {C₁ C₂ : Form}
      ...
      OccI d₁ s → OccI (FRJVi.orI d₁ d₂ h₁ h₂ hg hSt hTh) s
  | orI₂ {St₁ Th₁ St₂ Th₂ : List Form} {C₁ C₂ : Form}
      {d₁ : FRJVi G St₁ Th₁ C₁} {d₂ : FRJVi G St₂ Th₂ C₂}
      {h₁ : St₁ ⊆ St₂ ++ Th₂} {h₂ : St₂ ⊆ St₁ ++ Th₁}
      {hg : Form.or C₁ C₂ ∈ sfR G} {St' Th' : List Form} {hSt : St' ≐ St₁ ++ St₂} {hTh : Th' ≐ cap Th₁ Th₂} {s : Sequent} :
  ...
  | impInI {St Th Lam ThLam : List Form} {A B : Form}
      ...
      OccI d s → OccI (FRJVi.impInI d hpre hdisj hA hg hSt hTh) s
  | impNotIn {t : Tag} {Γ Th : List Form} {A B : Form} {d : FRJVr G t Γ B}
      {hTh : ∀ X ∈ Th, Clo Γ X ∧ X ∈ gHat G} {hA : Clo Γ A} {hAnot : ¬ Clo Th A}
      {hg : Form.imp A B ∈ sfR G} {s : Sequent} :
      OccR d s → OccI (FRJVi.impNotIn d hTh hA hAnot hg) s
  | circNotIn {t : Tag} {Γ Th : List Form} {Z : Form} {d : FRJVr G t Γ Z}
      {htag : t = .barren ∨ ∃ W, t = .chain W ∧ Covers Γ W Z}
      {hTh : ∀ X ∈ Th, Clo Γ X ∧ X ∈ gHat G}
      {hg : Form.circ Z ∈ sfR G} {s : Sequent} :
...
theorem occI_steps {G : Form} {St Th : List Form} {C : Form}
    {d : FRJVi G St Th C} {s : Sequent} : OccI d s → StepsRfl G s (.irr St Th C)
  | .root _ => .refl
...
theorem lhs_clo_of_occR {G : Form} {t : Tag} {Γ : List Form} {C : Form}
    {d : FRJVr G t Γ C} {s : Sequent} (h : OccR d s) :
    ∀ X ∈ Γ, Clo s.lhs X :=
...
the V-joins the kept zone is inside `th 0`, so `wfI (prem 0)` covers it
...
theorem atPart_union_impPart {G : Form} {t : Tag} {Γ : List Form} {C : Form}
    (d : FRJVr G t Γ C) : Γ ⊆ atPart Γ ++ impPart Γ ++ circPart Γ := by
  intro x hx
  have hG := wfR d hx
