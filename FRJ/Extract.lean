...
def RegIdx {G : Form} : {St Th : List Form} → {C : Form} → FRJi G St Th C → Type
  | _, _, _, .axI _ _ _ rfl => Empty
  | _, _, _, .andI1 d _ => RegIdx d
  | _, _, _, .andI2 d _ => RegIdx d
  | _, _, _, .orI d₁ d₂ _ _ _ rfl rfl => Sum (RegIdx d₁) (RegIdx d₂)
  | _, _, _, .impInI d _ _ _ rfl rfl => RegIdx d
  | _, _, _, .impNotIn _ _ _ _ _ => Unit
  | _, _, _, .circNotIn _ _ _ _ => Unit
  | _, _, _, .axIC _ _ _ _ _ rfl => Unit
...
theorem regIdxComplete {G : Form} : ∀ {St Th : List Form} {C : Form}
  ...
  | _, _, _, .impInI d _ _ _ rfl rfl, i => regIdxComplete d i
  | _, _, _, .impNotIn _ _ _ _ _, _ => List.mem_cons_self
  | _, _, _, .circNotIn _ _ _ _, _ => List.mem_cons_self
  | _, _, _, .axIC _ _ _ _ _ rfl, _ => List.mem_cons_self
...
def preR {G : Form} : {t : Tag} → {Γ : List Form} → {C : Form} → FRJr G t Γ C → PreModel
  | _, _, _, .axR F _ _ rfl => PreModel.leaf (rm (gAt G) F)
  | _, _, _, .andR1 d _ => preR d
  | _, _, _, .andR2 d _ => preR d
  | _, _, _, .impIn d _ _ => preR d
  | _, _, _, .circIn d _ _ => preR d
  | _, _, _, @FRJr.joinAt _ n stab th rhs F prem _ _ _ _ _ _ _ rfl =>
      PreModel.join (premIdxElems prem) (premIdxComplete prem)
        ...
        (fun _ => false)
  | _, _, _, @FRJr.joinAtP _ n k stab th rhs F _ tps Δs Ds prem dps _ _ _ _ _ _ _ _ _ rfl =>
  ...
  | _, _, _, @FRJr.joinAtF _ n stab th rhs F prem _ _ _ _ _ _ rfl =>
      PreModel.join (sumElems (premIdxElems prem) [()])
        ...
        (Sum.elim (fun _ => false) (fun _ => true))
  | _, _, _, @FRJr.joinOr _ n stab th rhs C₁ C₂ prem _ _ _ _ _ _ rfl =>
      PreModel.join (premIdxElems prem) (premIdxComplete prem)
        (joinCtxOr stab th rhs)
        (fun (ji : (j : Fin (n + 1)) × RegIdx (prem j)) => preI (prem ji.1) ji.2)
        (fun _ => false)
  | _, _, _, @FRJr.joinOrP _ n k stab th rhs C₁ C₂ _ tps Δs Ds prem dps _ _ _ _ _ _ _ _ rfl =>
      PreModel.join (sumElems (premIdxElems prem) (List.finRange (k + 1)))
        (sumElems_complete (premIdxComplete prem) List.mem_finRange)
        ...
        (Sum.elim
          ...
          (fun i => preR (dps i)))
        (Sum.elim (fun _ => false) (fun _ => true))
  | _, _, _, @FRJr.joinOrF _ n stab th rhs C₁ C₂ prem _ _ _ _ _ rfl =>
      PreModel.join (sumElems (premIdxElems prem) [()])
        (sumElems_complete (premIdxComplete prem) (fun _ => List.mem_cons_self))
        ...
        (Sum.elim (fun _ => false) (fun _ => true))
  | _, _, _, @FRJr.joinCirc _ n stab th rhs Z prem _ _ _ _ _ _ rfl =>
      PreModel.join (premIdxElems prem) (premIdxComplete prem)
        (joinCtxOr stab th rhs)
        (fun (ji : (j : Fin (n + 1)) × RegIdx (prem j)) => preI (prem ji.1) ji.2)
        (fun _ => false)
  | _, _, _, @FRJr.joinCircP _ n k stab th rhs Z tps Δs Ds prem dps _ _ _ _ _ _ _ _ rfl =>
      PreModel.join (sumElems (premIdxElems prem) (List.finRange (k + 1)))
        (sumElems_complete (premIdxComplete prem) List.mem_finRange)
        (joinCtxOrP stab th rhs Δs)
        (Sum.elim
...
def preI {G : Form} : {St Th : List Form} → {C : Form} →
  ...
  | _, _, _, .axI _ _ _ rfl, i => (i : Empty).elim
  | _, _, _, .andI1 d _, i => preI d i
  | _, _, _, .andI2 d _, i => preI d i
  | _, _, _, .orI d₁ d₂ _ _ _ rfl rfl, i =>
      match (i : Sum (RegIdx d₁) (RegIdx d₂)) with
      | .inl i₁ => preI d₁ i₁
      | .inr i₂ => preI d₂ i₂
  | _, _, _, .impInI d _ _ _ rfl rfl, i => preI d i
  | _, _, _, .impNotIn d _ _ _ _, _ => preR d
  | _, _, _, .circNotIn d _ _ _, _ => preR d
  | _, _, _, .axIC _ ats _ _ _ rfl, _ => PreModel.leaf (vacZoneA G ats)
...
theorem preI_spec {G : Form} : ∀ {St Th : List Form} {C : Form}
  ...
  | _, _, _, .orI d₁ d₂ _ _ _ rfl rfl, i => by
      ...
      | .inr i₂ =>
          obtain ⟨s, hocc, hlbl⟩ := preI_spec d₂ i₂
          exact ⟨s, .orI₂ hocc, hlbl⟩
  | _, _, _, .impInI d _ _ _ rfl rfl, i => by
      obtain ⟨s, hocc, hlbl⟩ := preI_spec d i
...
theorem preR_closed {G : Form} : ∀ {t : Tag} {Γ : List Form} {C : Form}
  ...
  | _, _, _, .axR _ _ _ rfl => fun _ _ _ X hX => .base hX
  ...
  | _, _, _, @FRJr.joinAtP _ n k stab th rhs F t' tps Δs Ds prem dps hJ1 _ _ hJ7 _ _ _ _ _ rfl => by
      intro w v hle X hX
      cases v with
      | none => cases hle with
        | root => exact .base hX
  ...
  | _, _, _, @FRJr.joinAtF _ n stab th rhs F prem hJ1 _ _ _ _ _ rfl => by
      intro w v hle X hX
      cases v with
      | none => cases hle with
        | root => exact .base hX
      | some jb =>
          ...
          | comp hab =>
              ...
              | inl ji => exact preI_closed (prem ji.1) ji.2 _ _ hab X hX
              | inr _ => exact .base hX
  | _, _, _, @FRJr.joinOrP _ n k stab th rhs C₁ C₂ t' tps Δs Ds prem dps hJ1 _ _ hJ7 _ _ _ _ rfl => by
      intro w v hle X hX
      cases v with
      | none => cases hle with
        | root => exact .base hX
      | some jb =>
          ...
          | comp hab =>
              ...
              | inl ji => exact preI_closed (prem ji.1) ji.2 _ _ hab X hX
              | inr i => exact preR_closed (dps i) _ _ hab X hX
  | _, _, _, @FRJr.joinOrF _ n stab th rhs C₁ C₂ prem hJ1 _ _ _ _ rfl => by
      intro w v hle X hX
      cases v with
      | none => cases hle with
        | root => exact .base hX
      | some jb =>
          ...
          | comp hab =>
              ...
              | inl ji => exact preI_closed (prem ji.1) ji.2 _ _ hab X hX
              | inr _ => exact .base hX
  | _, _, _, @FRJr.joinCircP _ n k stab th rhs Z tps Δs Ds prem dps hJ1 _ _ _ _ _ _ _ rfl => by
      intro w v hle X hX
      cases v with
      | none => cases hle with
        | root => exact .base hX
      | some jb =>
          ...
          | comp hab =>
              ...
              | inl ji => exact preI_closed (prem ji.1) ji.2 _ _ hab X hX
              | inr i => exact preR_closed (dps i) _ _ hab X hX
  | _, _, _, @FRJr.joinCirc _ n stab th rhs Z prem hJ1 _ _ _ _ _ rfl => by
      intro w v hle X hX
      cases v with
      | none => cases hle with
        | root => exact .base hX
  ...
  | _, _, _, @FRJr.joinAt _ n stab th rhs F prem hJ1 _ _ _ _ _ _ rfl => by
      intro w v hle X hX
      cases v with
      | none => cases hle with
        | root => exact .base hX
      | some jb =>
          ...
          | root =>
              ...
              rw [hlbl]; exact hY
          | comp hab => exact preI_closed (prem _) _ _ _ hab X hX
  | _, _, _, @FRJr.joinOr _ n stab th rhs C₁ C₂ prem hJ1 _ _ _ _ _ rfl => by
      intro w v hle X hX
      cases v with
      | none => cases hle with
        | root => exact .base hX
...
theorem preI_closed {G : Form} : ∀ {St Th : List Form} {C : Form}
  ...
  | _, _, _, .impInI d _ _ _ rfl rfl, i => preI_closed d i
  | _, _, _, .impNotIn d _ _ _ _, _ => preR_closed d
  | _, _, _, .circNotIn d _ _ _, _ => preR_closed d
  | _, _, _, .axIC _ _ _ _ _ rfl, _ => fun _ _ _ X hX => .base hX
