/-
# The repaired calculus derives the ρ-order cell `[ρ20] ⊢? ρ13` — negative
...
def Θax : List Form := FRJ.rm (gAt G2013) .bot ++ gImp G2013 ++ gCirc G2013

def R1 : FRJVi G2013 [] Θax .bot :=
  .axI .bot (by decide) (by decide) (CtxEq.refl _)

def Ta : FRJVi G2013 [aF] (FRJ.sdiff Θax [aF]) naF :=
...
def R3a : FRJVr G2013 .blocked Γ3a .bot :=
  .joinAtF (n := 0) (F := .bot)
...
def i_na : FRJVi G2013 [] [bF, r8, r20] naF :=
...
def R2 : FRJVr G2013 .barren Γ2 .bot :=
...
def rowia : IRow G2013 := ⟨[], [bF, r8, r20], aF, i_a⟩
def rowina : IRow G2013 := ⟨[], [bF, r8, r20], naF, i_na⟩
def rowinna : IRow G2013 := ⟨[], [bF, r8, r20], nnaF, i_nna⟩
def rowi11s : IRow G2013 :=
...
def Q : FRJVr G2013 (.chain .bot) ΓQ r4 :=
...
def ROOT := FRJVr.impIn (G := G2013) (A := r8) (B := r4) Q
    (by decide) (by decide)

def goal := FRJVr.impIn (G := G2013) (A := r20) (B := r13) ROOT
    (by decide) (by decide)
...
`ρ20 ⊢? ρ13`** — hand-built. -/
theorem provableV_2013 : FRJ.ProvableV G2013 := ⟨.chain .bot, _, ⟨W2013.goal⟩⟩

/-- info: 'FRJ.WitnessV2013.provableV_2013' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms provableV_2013

end FRJ.WitnessV2013
