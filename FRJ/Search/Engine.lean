...
Ported verbatim from `wip/frj_sat.lean` (namespace `FRJSat`), which is the
FROZEN reference implementation: it is the differential oracle for
everything built here, and the two are expected to agree on every goal
both settle.  That file is part of the working record and is not
distributed on this branch; it is on the development branches.
