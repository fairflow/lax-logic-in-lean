/-
V-adaptation of `FRJ/Extract.lean`; the three barren joins carry a kept
zone — see `FRJ/CalculusV.lean`.

# `Mod(D)`: the labelled model extracted from a derivation

Section 3.1, transported to the repaired family `FRJVr`/`FRJVi`.  The
...
conclusion context `joinCtxAtVBase stab th F ++ kept` (resp.
`joinCtxOrVBase stab th ++ kept`), which is exactly what `hΓ` says `Γ'`
...
lies in `Θ^⊃∩ ⊆ th j`), so the closure argument here is verbatim the
...
def RegIdx {G : Form} : {St Th : List Form} → {C : Form} → FRJVi G St Th C → Type
...
instance regIdxDecEq {G : Form} : ∀ {St Th : List Form} {C : Form}
    (d : FRJVi G St Th C), DecidableEq (RegIdx d)
...
def regIdxElems {G : Form} : ∀ {St Th : List Form} {C : Form}
    (d : FRJVi G St Th C), List (RegIdx d)
  | _, _, _, .axI _ _ _ _ => []
...
theorem regIdxComplete {G : Form} : ∀ {St Th : List Form} {C : Form}
    (d : FRJVi G St Th C) (i : RegIdx d), i ∈ regIdxElems d
...
def preR {G : Form} : {t : Tag} → {Γ : List Form} → {C : Form} → FRJVr G t Γ C → PreModel
  ...
  | _, _, _, @FRJVr.joinAt _ n stab th _ F kept prem _ _ _ _ _ _ _ _ _ =>
      PreModel.join (premIdxElems prem) (premIdxComplete prem)
        ...
        (fun _ => false)
  | _, _, _, @FRJVr.joinAtP _ n k stab th rhs F _ tps Δs Ds prem dps _ _ _ _ _ _ _ _ _ _ =>
  ...
  | _, _, _, @FRJVr.joinAtF _ n stab th rhs F prem _ _ _ _ _ _ _ =>
      PreModel.join (sumElems (premIdxElems prem) [()])
        ...
        (Sum.elim (fun _ => false) (fun _ => true))
  | _, _, _, @FRJVr.joinOr _ n stab th _ C₁ C₂ kept prem _ _ _ _ _ _ _ _ =>
      PreModel.join (premIdxElems prem) (premIdxComplete prem)
        (joinCtxOrVBase stab th ++ kept)
        (fun (ji : (j : Fin (n + 1)) × RegIdx (prem j)) => preI (prem ji.1) ji.2)
        (fun _ => false)
  | _, _, _, @FRJVr.joinOrP _ n k stab th rhs C₁ C₂ _ tps Δs Ds prem dps _ _ _ _ _ _ _ _ _ =>
      PreModel.join (sumElems (premIdxElems prem) (List.finRange (k + 1)))
        (sumElems_complete (premIdxComplete prem) List.mem_finRange)
        ...
        (Sum.elim
          ...
          (fun i => preR (dps i)))
        (Sum.elim (fun _ => false) (fun _ => true))
  | _, _, _, @FRJVr.joinOrF _ n stab th rhs C₁ C₂ prem _ _ _ _ _ _ =>
      PreModel.join (sumElems (premIdxElems prem) [()])
        (sumElems_complete (premIdxComplete prem) (fun _ => List.mem_cons_self))
        ...
        (Sum.elim (fun _ => false) (fun _ => true))
  | _, _, _, @FRJVr.joinCirc _ n stab th _ Z kept prem _ _ _ _ _ _ _ _ =>
      PreModel.join (premIdxElems prem) (premIdxComplete prem)
        (joinCtxOrVBase stab th ++ kept)
        (fun (ji : (j : Fin (n + 1)) × RegIdx (prem j)) => preI (prem ji.1) ji.2)
        (fun _ => false)
  | _, _, _, @FRJVr.joinCircP _ n k stab th rhs Z tps Δs Ds prem dps _ _ _ _ _ _ _ _ _ =>
      PreModel.join (sumElems (premIdxElems prem) (List.finRange (k + 1)))
        (sumElems_complete (premIdxComplete prem) List.mem_finRange)
        (joinCtxOrP stab th rhs Δs)
        (Sum.elim
...
def preI {G : Form} : {St Th : List Form} → {C : Form} →
    (d : FRJVi G St Th C) → RegIdx d → PreModel
...
theorem preR_root_lbl {G : Form} : ∀ {t : Tag} {Γ : List Form} {C : Form}
    (d : FRJVr G t Γ C), (preR d).lbl (preR d).root ≐ Γ
...
theorem preR_closed {G : Form} : ∀ {t : Tag} {Γ : List Form} {C : Form}
    (d : FRJVr G t Γ C), ClosedLbl (preR d)
  ...
  | _, _, _, @FRJVr.joinAtP _ n k stab th rhs F t' tps Δs Ds prem dps hJ1 _ _ hJ7 _ _ _ _ _ _ => by
      intro w v hle X hX
      cases v with
      | none => cases hle with
        | root => exact .base hX
  ...
  | _, _, _, @FRJVr.joinAtF _ n stab th rhs F prem hJ1 _ _ _ _ _ _ => by
      intro w v hle X hX
      cases v with
      | none => cases hle with
        | root => exact .base hX
      | some jb =>
          ...
          | comp hab =>
              ...
              | inl ji => exact preI_closed (prem ji.1) ji.2 _ _ hab X hX
              | inr _ => exact .base hX
  | _, _, _, @FRJVr.joinOrP _ n k stab th rhs C₁ C₂ t' tps Δs Ds prem dps hJ1 _ _ hJ7 _ _ _ _ _ => by
      intro w v hle X hX
      cases v with
      | none => cases hle with
        | root => exact .base hX
      | some jb =>
          ...
          | comp hab =>
              ...
              | inl ji => exact preI_closed (prem ji.1) ji.2 _ _ hab X hX
              | inr i => exact preR_closed (dps i) _ _ hab X hX
  | _, _, _, @FRJVr.joinOrF _ n stab th rhs C₁ C₂ prem hJ1 _ _ _ _ _ => by
      intro w v hle X hX
      cases v with
      | none => cases hle with
        | root => exact .base hX
      | some jb =>
          ...
          | comp hab =>
              ...
              | inl ji => exact preI_closed (prem ji.1) ji.2 _ _ hab X hX
              | inr _ => exact .base hX
  | _, _, _, @FRJVr.joinCircP _ n k stab th rhs Z tps Δs Ds prem dps hJ1 _ _ _ _ _ _ _ _ => by
      intro w v hle X hX
      cases v with
      | none => cases hle with
        | root => exact .base hX
      | some jb =>
          ...
          | comp hab =>
              ...
              | inl ji => exact preI_closed (prem ji.1) ji.2 _ _ hab X hX
              | inr i => exact preR_closed (dps i) _ _ hab X hX
  | _, _, _, @FRJVr.joinCirc _ n stab th rhs Z kept prem hJ1 _ _ hkc _ _ _ _ => by
      intro w v hle X hX
      cases v with
      | none => cases hle with
        | root => exact .base hX
  ...
  | _, _, _, @FRJVr.joinAt _ n stab th rhs F kept prem hJ1 _ _ hkc _ _ _ _ _ => by
      intro w v hle X hX
      cases v with
      | none => cases hle with
        | root => exact .base hX
      | some jb =>
          ...
          | root =>
              ...
              rw [hlbl]; exact hY
          | comp hab => exact preI_closed (prem _) _ _ _ hab X hX
  | _, _, _, @FRJVr.joinOr _ n stab th rhs C₁ C₂ kept prem hJ1 _ _ hkc _ _ _ _ => by
      intro w v hle X hX
      cases v with
      | none => cases hle with
        | root => exact .base hX
...
theorem preI_closed {G : Form} : ∀ {St Th : List Form} {C : Form}
    (d : FRJVi G St Th C) (i : RegIdx d), ClosedLbl (preI d i)
  | _, _, _, .axI _ _ _ _, i => (i : Empty).elim
