...
| tool | use | version |
|---|---|---|
| `Rewrite.simplifyWith Rewrite.fullSetC fuel φ` (`Rewrite/`) | **Normalise before you search** — every fresh probe pipes its cells through this certified simpset first. Use `simplifyWith` against `fullSetC` (the canonicalised set), never `simplify`/`norm` or `fullSet`. Correctness is unconditional (`simplifyWith_interd`); the choice is effectiveness only. Measured by `lake exe rwscreen`. **Canonicaliser passes added 2026-09-04** (`Rewrite/Canon.lean`, each with its `Interd` certificate): `stripBox` — under a box, every `◯` reachable through `∧`/`∨` is deleted (`◯(a ∨ ◯b) = ◯(a ∨ b)`, `◯(a ∧ ◯b) = ◯(a ∧ b)`; subsumes `◯◯φ = ◯φ`; NOT through `⊃` in either position, G4c-refuted); `◯⊥ ∨ ◯ψ = ◯ψ` and `◯⊥ ∧ ◯ψ = ◯⊥` (`dropBoxBotIf`/`collapseBoxBotAnd`, via the syntactic absorber test `absorbsBoxBot`); chain-head deduplication in `insAnd`/`insOr` (a defect: `r ∨ (r ∨ t)` survived before); `simpRounds` 4 → 32, since each round strips one box level. Effect on the fuel chains: `docs/ui-ljfo-clause-table.md` §4.9. Pins: `#print axioms rndSet`/`fullSet` are `#guard_msgs`-guarded in `Rewrite/Catalogue.lean` — keep them pinned. **Pegged to R** (standing, 2026-08-26): when R grows, the new class's kernel-proved `Interd` cells join the set; the set never shrinks; promotion runs through `lake exe rcellsgen` (G4c-certificate route); queue as of 2026-08-26: the two ρ20/ρ21 forward cells. | `8c9ae91` · 2026-09-04 |
...
| G4c engines: `PLLND.Search.prove?Bounded` / `refute?` (`LaxLogic/PLL/Search/Search.lean`) | Premise-loaded (PCLL/`DerivU`) work, and `Config.findBudget` for grind-prone cells. Certificate-carrying, untrusted-but-safe, discover-then-pin. | `b0f4ca3` · 2026-08-19 |
...
| FinCM certificates: `FinCM.not_provable_of_check` (`LaxLogic/PLL/Semantics/CountermodelEmit.lean`) | Kernel-`decide` escalation of any found model: pin the frame, the theorem does the rest. Repo pin convention: witness world `w`, fallible worlds listed explicitly. | `b0f4ca3` · 2026-08-19 |
| Confluent battery: `RNC.not_derivU_of_checkConf` (`LaxLogic/PLL/Search/SearchConf.lean`) | Sweep a *known* frame battery as a check; sound, not complete (confluent models are a proper subclass). **Battery enumeration is not a discovery method** — it survives only as an independent check on a model an engine constructed. | `267fc1d` · 2026-07-27 |
...
| `lake build Production` / `lake build Experimental` | The two estates (`Audit/Production.lean`, `Audit/Experimental.lean`). Production FORBIDS `sorryAx`; promotion into it is mechanical — import the module and survive the sweep. Experimental ALLOWS it, and is where the ax-prover harness lives. Note `lake build` alone covers only `defaultTargets` and checks no axioms at all: that gap is why five `wip/` modules sat broken for weeks, and why a UI-line refutation (`wip/coverfail.lean`) was silently unverified. Held out of Production as of 2026-09-04: `LaxLogic.Belief.Examples` (two `native_decide` cardinalities) and the three `PLLSemUI*` modules (five sorried declarations from the shelved semantic-UI route). | `ca3c5f1 · 2026-09-04` |
