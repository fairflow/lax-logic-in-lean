(* catpart.lex : sample lexer for catpart *)
...
<INITIAL>Function   => (YYBEGIN MAIN; print"Into main..."; FUN(!line,!line));
<INITIAL>{ws}+	    => (lex());
...
<MAIN>\*{space}*    => (YYBEGIN CHOICE; print"Into choice.."; STAR(!line,!line));  
...
<MAIN>.	            => (error ("ignoring illegal character" ^ yytext,
			   !line,!line); lex());
<CHOICE>\[          => (YYBEGIN MAIN;  print"Into main...";  LSQBR(!line,!line));
<CHOICE>{newline}   => (YYBEGIN MAIN;  print"Into main..."; NEWLINE(!line,!line));
