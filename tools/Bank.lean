...
!! STATUS TAGS WITHDRAWN (2026-08-21) !!  Every `proved`/`refuted`/`open`
tag below was generated from `wip/rnDict.lean`, whose bookkeeping is
withdrawn: a `sorry`ed cell theorem is indistinguishable by name from a
proved one, so no tally derived from it survives.  Treat every tag as
UNKNOWN until regenerated against a rebuilt dictionary.  The SHAPE is
sound and is what a rebuild should keep — status as three-valued DATA
rather than as theorems — and the individual kernel-checked countermodels
(`wip/rnFRJCerts.lean`) stand on their own, since a countermodel is
self-certifying.  What is withdrawn is the tags, not the mechanism.
