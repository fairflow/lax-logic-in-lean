...
proved about them, and their statements are the author's to sign off. What
follows is a record of the design as it stands, with the semantic
obligations each rule would have to discharge; all of these obligations
are proved (Appendix~\ref{app:archive}).

\paragraph{The difficulty, in one sentence.} For implication the
obligation at a new world is \emph{negative} --- the antecedent fails
there, so the implication holds vacuously --- which is all the support
condition has to arrange. For the modality the obligation is
\emph{positive}: a modal witness must exist, and nothing in the
calculus's data supplies one.

\paragraph{Three screens.} A two-world model separates $\bigcirc p$ from
$p$, so a rule must be able to build a world whose modal successor forces
something the world itself does not; a three-world branching model refutes
the distribution of the modality over disjunction, so the witness is
attached per-world and not once per derivation; and a fallible world
forces everything, so a fallible witness is all-or-nothing. The second
screen also corrected a lemma: the first form written down, that a
fallible modal successor at a world forces every modal formula there, is
\emph{false}, because it says nothing about the modal successors of a
world strictly above.

\paragraph{The design.} A regular modal introduction, gated by
\emph{barrenness} --- the world declared no modal successor, so a modal
formula fails there as soon as its argument does --- together with a
\emph{promise join}, carrying one extra regular premise whose world
becomes the modal successor, and a \emph{fallible} variant.

\paragraph{A gap, and why it matters.} Building the first modal exemplar,
the refutation of $\bigcirc p \supset p$, showed the first two rules
cannot build it. The countermodel needs a world forcing $p$; at such a
world the modality holds by its unit, hence so does the goal, and
falsehood is not a right subformula of this goal --- so that world refutes
\emph{nothing} available to the calculus. Every world of the extracted
model is a p-sequent, and a p-sequent refutes its own goal by
construction. The witness therefore cannot be a p-sequent at all: it is a
\emph{fallible} world, which forces everything and refutes nothing. This
argument is about the extracted model and the right subformulas, so it
survives any change of encoding.
...
\S\S\ref{sec:prelim}--\ref{sec:choice}, for modality-free goals, together
with the extension of \S\ref{sec:w1}; and the six semantic obligations of
\S\ref{sec:rules}.

\textsc{Open}: soundness and completeness for any calculus containing
modal rules. No modal rule is part of the calculus yet.
...
than deleted. Its modal semantics and its screens are the source of
\S\ref{sec:rules}, and are reproduced here because the obligations proved
in them are what any modal rule must discharge.

\subsection{The modal semantics and the screens}
\label{app:modal}
\lstinputlisting{../../Archive/FRJLax-parallel/Modal.lean}
