...
`wip/rnSep.lean` and `wip/rnSepColl.lean` are imported for the worked
examples.  `wip/rnSep.lean` imports `wip/rnDict.lean`, which carries the
87 sorried cells — so `sorryAx` IS in this module's environment, and by
the paragraph above that is not a formality.  The pins at the foot of
the file are the machine check that no declaration here reaches one of
those cells.  Nothing in this file cites a sorried theorem.
-/
...
Transcribed verbatim from `lake env lean RNDB/Types.lean`.  A pin here
is a machine check that the declaration did not silently reach one of
the 87 sorried cells of `wip/rnDict.lean`, which ARE in this module's
environment via the import chain. -/
